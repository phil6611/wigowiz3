...
/*
 * Ce fichier sert pour créer un nouvel événement.
 */
...
if($envoi == 'TRUE'){
    ...
    if( !empty($titre_sql) && !empty($date_sql) && $test_date == TRUE){
        ...
        */
        ...
        //Codes pour l'administration de l'événement.
        $code = uniqid();
        $code_admin = uniqid('',true);
        
        //Tableau des variables pour les requêtes SQL.
        $array_variables = [
            ":id_evenement" => NULL,
            ":id_createur" => $_SESSION["compte"]["id_participant"],
            ":titre" => $titre_sql,
        ...
        $sql_evenement_prepare = "INSERT INTO `".PREFIXE."evenements`"
                . "(`id_evenement`, `id_createur`, `titre`, `description`, `date_creation`, `adresse`, `cp`, `ville`, `pays`, `date_evenement`, `date_fin`, `latitude`, `longitude`, `envoi_alerte`, `ip`, `code`, `code_admin`) "
                . "VALUES "
                . "(:id_evenement,:id_createur,:titre,:description,NOW(),:adresse,:cp,:ville,:pays,:date_evenement, :date_fin, :latitude,:longitude,:alerte,:ip,:code,:code_admin)";
        //Exécution de la requête.
        $resultat_evenement = $dbh->prepare($sql_evenement_prepare);

        try {
            $resultat_evenement -> execute($array_variables);
            //Récupération de l'identifiant de l'évenement.
            $id_evenement =  $dbh->lastInsertId();
            ...
            if ($je_participe_sql == 1){
                ...
                $sql_relation = "INSERT INTO `".PREFIXE."relations`(`id_relation`, `id_evenement`, `id_participant`,`distance`, `commentaire`)"
